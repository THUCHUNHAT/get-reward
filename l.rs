...
mod token {
    ...
    impl Token {
        ...
        pub fn get_reward(&mut self,t0_amount:u128) ->Result<u128>{
            let caller = self.env().caller();
            assert_eq!(caller, self.env().caller(), "Unauthorized");

            let required_t0:u128 =5;
            let reward_amount: u128=10;

            if t0_amount < required_t0{
                return Err(Error::RewardClaimFailed);
            }
        }
